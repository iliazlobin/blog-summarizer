...
def extract_insights(event, context):
    ...
    insights = get_blog_post_insights(title, text, "GCP")
...
def upsert_document(event, context):
    ...
    document = event
    document["blog"] = event["tag"]
    document["date"] = event["date"].split("T")[0]
    document["authors"] = [author["name"] for author in event["authors"]]
    result = upsert_blog_database_document("GCP", document)
